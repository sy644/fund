...
function renderSummary() {
  ...
  var stats = state.map(f => {
    ...
    var sh = initShares + f.buys.reduce((s,b)=>s+(b.amount/(b.price||1)),0);
    var mv = curPrice * sh;
    var pnl = mv-inv;
  ...
  stats.forEach(s => {
    ...
    var dc = s.dropPct > 0 ? '#dc2626' : (s.dropPct < 0 ? '#16a34a' : '#93A3BD');
    var dropStr = (s.dropPct>=0?'+':'') + s.dropPct.toFixed(1) + '%';
    html += '<tr>';
...
function showExportModal() {
  ...
  var stats = state.map(f => {
    ...
    var shares = f.initShares + f.buys.reduce((s,b) => s + (b.amount/(b.price||1)), 0);
...
function exportExcelToFile() {
  ...
  var totalShares = state.reduce((s,f)=>{
    var inv=f.buys.reduce((s,b)=>s+(b.amount||0),0);
...
// 自定义 modal - 手动记录基金净值
var NAV_HISTORY_KEY = 'nav_history';
