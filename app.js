...
function bindFundEvents(f, i) {
  ...
  // OCR 识图按钮
  const ocrBtn = document.getElementById(`ocr-${i}`);
  if (ocrBtn) {
    ocrBtn.onclick = () => {
      // 用 hidden file input 选择图片
      let input = document.getElementById('ocrFileInput');
      if (!input) {
  ...
  f.buys.forEach((b, bi) => {
    ...
    const refreshShares = () => {
      ...
      const matched = b.date ? (navHistory.find(r => r.code === f.code && r.date === b.date) || {}).nav : null;
      const sh = (absAmt && matched) ? (absAmt / matched) : 0;
    ...
    if (dateInp) {
      ...
      const updateDateOverlay = () => {
        ...
        let ovl = parent.querySelector('.bdate-overlay');
        if (!ovl) {
      ...
      const triggerMissingPrompt = (v) => {
        if (!v) return;
        const navHistory = (() => { try { return JSON.parse(localStorage.getItem('nav_history') || '[]'); } catch(e) { return []; }})();
        const found = navHistory.find(r => r.code === f.code && r.date === v);
        if (!found) {
          // 用 sessionStorage 避免同一次会话对同一日期+基金重复弹
          const key = 'miss_' + f.code + '_' + v;
      ...
      const firstCheck = () => {
        if (!b.date) return;
        const navHistory = (() => { try { return JSON.parse(localStorage.getItem('nav_history') || '[]'); } catch(e) { return []; }})();
        const found = navHistory.find(r => r.code === f.code && r.date === b.date);
    ...
    if (sdayInp) {
      ...
      sdayInp.addEventListener('change', e => {
        ...
        const navHistory2 = (() => { try { return JSON.parse(localStorage.getItem('nav_history') || '[]'); } catch(e) { return []; }})();
        const bNavMatch = b.date ? (navHistory2.find(r => r.code === f.code && r.date === b.date) || {}).nav : null;
        const priceBuy = bNavMatch != null ? bNavMatch : 0;
        const refPrice = sdayNav != null ? sdayNav : (bNavMatch != null ? bNavMatch : priceNow);
...
function renderFund(f, i) {
  ...
  if (Array.isArray(f.buys)) {
    f.buys = f.buys.slice().sort((a, b) => {
      const ad = a.date || a.sday || '';
      const bd = b.date || b.sday || '';
        ...
        <div class="buy-table-wrap">
          ...
          <div class="buy-grid-body">
              ${f.buys.map((b, bi) => {  // 保持原序, 排序由另一处处理
                ...
                const bNavMatch = b.date ? (navHistory.find(r => r.code === f.code && r.date === b.date) || {}).nav : null;
                // 份额 = 金额绝对值 / 净值(从 b.date 匹配)
                const shares = (realAmt && bNavMatch) ? (Math.abs(realAmt) / bNavMatch) : 0;
                // 涨幅: 优先用 (Sday净值 - 净值)/净值, 否则 (现价 - 净值)/净值
                const priceNow = f.price || 0;
                const priceBuy = bNavMatch != null ? bNavMatch : 0;
                const refPrice = sdayNav != null ? sdayNav : (bNavMatch != null ? bNavMatch : priceNow);
                let changePct = null;
...
// ============== Bday 智能日期 (交易时间→交易日) ==============
// A股交易时间: 周一-周五 9:30-11:30, 13:00-15:00 (节假日不开盘)
// 规则: 买入时刻在当天 9:30-15:00 → Bday = 当天; 15:00 后 → Bday = 之后最近交易日; 9:30 前 → 跨日, 算前一天
// 简化: 用 "如果买入时间 ≥ 15:00" → Bday 是之后第一个交易日, 否则当天
const CN_HOLIDAYS_2026 = ['2026-01-01','2026-01-02','2026-02-16','2026-02-17','2026-02-18','2026-02-19','2026-02-20','2026-04-06','2026-05-01','2026-05-04','2026-05-05','2026-06-19','2026-09-25','2026-10-01','2026-10-02','2026-10-05','2026-10-06','2026-10-07','2026-10-08','2026-12-25'];
...
function parseBuyRecords(text) {
  ...
  const rows = [];
  // 匹配日期+时间
  const reDateTime = /(20\d{2})[\-\/年.](\d{1,2})[\-\/月.](\d{1,2})\s+(\d{1,2}):(\d{2})(?::(\d{2}))?/;
  // 匹配金额 (整数或带小数)
  const reAmount = /(\d{1,6}(?:,\d{3})*(?:\.\d{1,2})?)\s*元?/g;
  for (const line of lines) {
    const dm = line.match(reDateTime);
    if (!dm) continue;
    const date = `${dm[1]}-${dm[2].padStart(2,'0')}-${dm[3].padStart(2,'0')}`;
    const time = `${dm[4].padStart(2,'0')}:${dm[5]}:${dm[6] || '00'}`;
    // 找金额: 在同一行或下一行找 "数字元" 模式
    let amount = null;
    const amts = line.match(/(\d+(?:\.\d+)?)\s*元/g) || [];
    for (const a of amts) {
      const v = parseFloat(a.replace(/[^\d.]/g, ''));
...
async function runOCR(file, f, i) {
  ...
  try {
    ...
    const records = parseBuyRecords(text);
    if (records.length === 0) {
      showToast('❌ 未识别到交易记录');
      return;
    }
    // 用 smartBday 计算 Bday
    const enriched = records.map(r => ({ ...r, bday: smartBday(r.date, r.time) }));
    // 显示识别结果让用户确认
    showOCRConfirmDialog(f, i, enriched);
  } catch(err) {
...
function showOCRConfirmDialog(f, i, records) {
  ...
  const rowsHtml = records.map((r, idx) => {
    const bdayChanged = r.bday !== r.date;
    return `<div style="display:flex;gap:8px;align-items:center;padding:8px;background:rgba(0,0,0,0.3);border-radius:8px;margin-bottom:6px;font-size:12px">
