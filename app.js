...
function generateAdvice(f, i) {
  var price = parseFloat(f.price) || 0;
  var low = parseFloat(f.priceLow) || 0;
  var mid = parseFloat(f.priceMid) || 0;
  var high = parseFloat(f.priceHigh) || 0;
  if (!price || !low || !high || low >= high) return '';

  // 相对位置 (0~100%)
  var pos = ((price - low) / (high - low) * 100);
  // 修复幅度 (相对低点的涨幅)
  var fix = ((price - low) / low * 100);
  // 下一档位（使用 calcCurrent 里的 neighbors）
  var { neighbors } = calcCurrent(f);
  var nextTier = null;
  if (neighbors && neighbors.length > 0) {
    // 找离当前价格最近的下一档（价格大于现价的第一个）
    var sorted = neighbors.slice().sort((a, b) => a.trigger - b.trigger);
    for (var n of sorted) {
      if (n.trigger > price) {
        nextTier = n;
        break;
    ...
    // 如果所有档位都低于现价（即已跌破所有档），则取最高档
    if (!nextTier) nextTier = sorted[sorted.length - 1];
  }
  ...
  advHtml += `<div class="advice-box" style="background:rgba(0,240,255,0.04);border:1px solid rgba(0,240,255,0.2);border-radius:12px;padding:12px;margin:10px 0;font-size:13px;line-height:1.8">`;
  advHtml += `<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">`;
  advHtml += `<span style="font-weight:800;color:#00f0ff">📊 综合判断</span>`;
  advHtml += `<span style="font-weight:700;color:${color}">${status}</span>`;
  advHtml += `</div>`;
  advHtml += `<div style="display:grid;grid-template-columns:1fr 1fr;gap:4px 16px;font-size:12px">`;
  advHtml += `<span>现价 <b>${price.toFixed(4)}</b></span>`;
  advHtml += `<span>修复幅度 <b style="color:${fix>=0?'#22c55e':'#fb7185'}">${fix >= 0 ? '+' : ''}${fix.toFixed(1)}%</b></span>`;
  advHtml += `<span>区间位置 <b>${pos.toFixed(1)}%</b> (低→高)</span>`;
  advHtml += `<span>距低点 <b>${((price-low)/low*100).toFixed(1)}%</b></span>`;
  if (nextTier) {
    var diff = nextTier.trigger - price;
    var diffPct = (diff / price * 100);
    advHtml += `<span>下一档 <b>${nextTier.label}</b> ${nextTier.trigger.toFixed(4)}</span>`;
    advHtml += `<span>距离 <b style="color:${diffPct<2?'#facc15':'#93A3BD'}">${diffPct > 0 ? '+' : ''}${diffPct.toFixed(1)}%</b></span>`;
    advHtml += `<span style="grid-column:1/3">目标加仓价：<b style="color:#00f5c8">${nextTier.trigger.toFixed(4)}</b> (${nextTier.amt ? Math.round(nextTier.amt).toLocaleString() : '-'} 元)</span>`;
  }
  advHtml += `</div>`;
  advHtml += `</div>`;
  return advHtml;
...
// 在 return 的模板字符串里，跑道 div 之后，环形图之前，加上：
${generateAdvice(f, i)}
