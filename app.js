        ...
        <div class="buy-table-wrap">
          ...
          <div class="buy-grid-body">
              ${f.buys.map((b, bi) => {  // 保持原序, 排序由另一处处理
                ...
                const priceNow = f.price || 0;
                const priceBuy = bNavMatch != null ? bNavMatch : 0;
                const hasSdayMatch = !!(sday && sdayNav != null);
                const refPrice = hasSdayMatch ? sdayNav : priceNow;
                let changePct = null;
                ...
                // Sday 标灰: 有 sday 但没匹配
                const sdayMiss = !!(sday && sdayNav == null);
                // 日期: 转成 xx/xx 格式
                let dateShort = '';
                if (b.date) {
...
function parseBuyRecords(text) {
  ...
  // 匹配金额: 数字(带.或,)后可选 "元"
  const reAmount = /(\d{1,7}(?:,\d{3})*(?:\.\d{1,2})?)\s*元/;
...
async function runOCR(file, f, i) {
  ...
  try {
    ...
    const enriched = records.map(r => {
      const b = smartBday(r.date, r.time);
