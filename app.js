          ...
          return `
            <div class="range-bar-section">
              ...
              <div class="range-bar-wrap">
                <div class="range-bar-marks">
                  <span>${pLow.toFixed(4)}</span>
                  <span style="color:var(--neon-yellow)">${pMid.toFixed(4)} ⭐</span>
                  <span>${pHigh.toFixed(4)}</span>
                </div>
                <div class="range-bar">${(() => {
                  // 给每格上色: 现价所在格 = 现价颜色, 中点格 = 黄
                  var cells = '';
                  for (var k = 0; k < total; k++) {
                    var ch = chars[k];
                    var cls = 'rb-cell';
                    if (ch === '☀') cls += ' rb-end';
                    else if (k === nowCell && k === midCell) cls += ' rb-now rb-mid';
                    else if (k === nowCell) cls += ' rb-now';
                    else if (k === midCell) cls += ' rb-mid';
                    else if (ch === '♥') cls += ' rb-filled';
                    else cls += ' rb-empty';
                ...
                <div class="range-bar-labels">
                  <span>距低 ${distLow}%</span>
                  <span style="color:${nowColor};font-weight:800">现 ${pNow.toFixed(4)} · ${nowPct}%</span>
                  <span>距高 ${distHigh}%</span>
                </div>
              </div>
            </div>
          `;
