  ...
  f.buys.forEach((b, bi) => {
    ...
    if (sdayInp) {
      ...
      const updateSdayOverlay = () => {
        let ovl = sdayInp.parentElement.querySelector('.bdate-overlay');
        if (!ovl) {
          ovl = document.createElement('div');
          ovl.className = 'bdate-overlay';
          ovl.style.cssText = 'position:absolute;left:0;right:0;top:0;bottom:0;display:flex;align-items:center;justify-content:center;pointer-events:none;color:#00f0ff;font-weight:700;font-size:13px;letter-spacing:.5px;text-shadow:0 0 6px rgba(0,240,255,0.5)';
          sdayInp.parentElement.appendChild(ovl);
        }
      ...
      sdayInp.addEventListener('change', e => {
        ...
        const navSpan = document.querySelector(`[data-bi="${bi}"].bnav`);
        if (navSpan) navSpan.textContent = sdayNav != null ? sdayNav.toFixed(4) : '-';
        // 涨幅
    ...
    <div class="fund" style="border-top: 4px solid ${f.color}">
      ...
      <div class="buy-section">
        ...
        <div class="buy-table-wrap">
          <div class="buy-grid-head"><div>日期</div><div>价格</div><div>金额</div><div>份额</div><div>涨幅</div><div>Sday</div><div>净值</div></div>
          <div class="buy-grid-body">
            ...
            <div class="buy-row" data-bi="${bi}" data-fund-i="${i}">
              <div class="buy-row-inner">
                ...
                <div class="bc bc-pill bc-sday"><input type="date" id="bsday-${i}-${bi}" value="${sday}" data-short="${sdayShort}" class="bcell bdate-slider" data-bi="${bi}" data-fund-i="${i}"></div>
                <div class="bc bc-pill"><span class="bnav" data-bi="${bi}" style="color:var(--neon-green);font-size:12px;font-weight:700">${sdayNav != null ? sdayNav.toFixed(4) : '-'}</span></div>
              </div>
            </div>
              `;}).join('')}
          </div>
