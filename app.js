// 公共计算工具函数（统一收益/份额逻辑，消除多处重复代码）
function calcFundMetrics(fund) {
  const { initShares = 0, initCost = 0, buys = [], price = 0 } = fund;
  // 初始投入：初始份额 × 初始成本（不再复用基准价）
  const investInit = initShares * initCost;

  // 买入记录投入 & 有效份额，价格为0则跳过该笔
  let investBuy = 0;
  let shareBuy = 0;
  for (const b of buys) {
  ...
  const profitRate = totalInvest > 0 ? (pnl / totalInvest) * 100 : 0;
  const avgCost = totalShare > 0 ? totalInvest / totalShare : 0;
  const dropPct = fund.basePrice > 0 ? ((price - fund.basePrice) / fund.basePrice) * 100 : 0;
  const progress = fund.target > 0 ? (totalInvest / fund.target) * 100 : 0;
...
function buildTierTable(f) {
  const { target, initShares, multi, tiers, basePrice, priceLow, priceMid, priceHigh, step, initCost = 0 } = f;
...
function calcTier(f) {
  ...
  const dropPct = (price - basePrice) / basePrice;
  return { tier, dropPct };
}
...
function bindFundEvents(f, i) {
  ...
  ['base-basePrice', 'base-initShares', 'base-initCost', 'base-target'].forEach(k => {
  ...
  document.getElementById(`addBuy-${i}`)?.addEventListener('click', () => addBuyDialog(i));
  document.getElementById(`undo-${i}`)?.addEventListener('click', undo);
  document.getElementById(`redo-${i}`)?.addEventListener('click', redo);
...
function renderSummary() {
  ...
  const stats = state.map(f => {
    ...
    totalInv += m.totalInvest;
    totalVal += m.marketValue;
    totalTgt += f.target || 0;
    totalShares += m.totalShare;
    return { f, ...m };
  });
  const totalPnl = totalVal - totalInv;
  const totalRate = totalInv > 0 ? Number((totalPnl / totalInv * 100).toFixed(2)) : 0;
  ...
  stats.forEach(s => {
    ...
    html += '<td>' + Math.round(s.marketValue).toLocaleString() + '</td>';
    html += '<td>' + Math.round(s.totalShare).toLocaleString() + '</td>';
    html += '<td style="color:' + pc + '">' + (s.pnl >= 0 ? '+' : '') + Math.round(s.pnl).toLocaleString() + '</td>';
    html += '<td style="color:' + pc + '">' + s.profitRate.toFixed(1) + '%</td>';
    html += '<td>' + Math.round(s.totalInvest).toLocaleString() + '</td>';
    html += '<td>' + s.progress.toFixed(0) + '%</td>';
    html += '</tr>';
  });
  html += '<tr style="background:#1F4E78;color:#fff;font-weight:700"><td>合计</td><td>-</td><td>-</td><td>' + Math.round(totalVal).toLocaleString() + '</td><td>' + Math.round(totalShares).toLocaleString() + '</td><td style="color:#FFD700">' + (totalPnl >= 0 ? '+' : '') + Math.round(totalPnl).toLocaleString() + '</td><td style="color:#FFD700">' + totalRate + '%</td><td>' + Math.round(totalInv).toLocaleString() + '</td><td>' + (totalTgt > 0 ? (totalInv / totalTgt * 100).toFixed(0) : '0') + '%</td></tr>';
  ...
  stats.forEach(s => {
    ...
    const dropStr = (s.dropPct >= 0 ? '+' : '') + s.dropPct.toFixed(1) + '%';
    const dropColor = s.dropPct < -10 ? '#dc2626' : (s.dropPct < -3 ? '#f59e0b' : (s.dropPct > 0 ? '#16a34a' : '#93A3BD'));
    ...
    if (currentIsBuy) {
      ...
      reason = s.dropPct < -10 ? '已深度下跌，加仓区间' : (s.dropPct < 0 ? '回调至加仓点' : '走势偏弱');
      ...
      reason = s.dropPct < -15 ? '深度超跌，可考虑分批' : '下行中，耐心等';
    ...
    const indAdv = s.dropPct < -15 ? '🔻 超跌，可分批' : (s.dropPct < -5 ? '⚠️ 偏弱' : (s.dropPct < 0 ? '📉 弱市' : '📈 偏强'));
    ...
    html += '<div class="ac-row ac-foot"><span>投入 ' + Math.round(s.totalInvest).toLocaleString() + ' · 完成 ' + s.progress.toFixed(0) + '%</span><b>收益 ' + (s.pnl >= 0 ? '+' : '') + Math.round(s.pnl).toLocaleString() + ' (' + s.profitRate.toFixed(1) + '%)</b></div>';
    html += '</div></div>';
  });
...
function updateCardValues(i) {
  ...
  const dropPct = m.dropPct;
  const dropColor = dropPct < 0 ? '#dc2626' : '#16a34a';
  const pnlClass = m.pnl > 0 ? 'pnl-pos' : (m.pnl < 0 ? 'pnl-neg' : '');
  ...
  if (ringAmt) ringAmt.textContent = Math.round(m.totalInvest).toLocaleString() + ' / ' + f.target.toLocaleString();
  if (ringFoot) ringFoot.textContent = '剩余 ' + Math.max(0, f.target - m.totalInvest).toLocaleString();
  if (ringPct) ringPct.textContent = (m.progress * 100).toFixed(0) + '%';
  if (ringFill) {
    const C = 2 * Math.PI * 86;
    const pct = Math.min(1, m.progress);
    ringFill.setAttribute('stroke-dasharray', (C * pct).toFixed(1) + ' ' + C.toFixed(1));
  }
  const stats = card.querySelectorAll('.fund-stats > div .val');
  if (stats[0]) stats[0].textContent = Math.round(m.marketValue).toLocaleString();
  if (stats[1]) stats[1].textContent = Math.round(m.totalShare).toLocaleString();
  if (stats[2]) stats[2].textContent = m.avgCost > 0 ? m.avgCost.toFixed(4) : '-';
  if (stats[3]) { stats[3].textContent = (m.pnl >= 0 ? '+' : '') + Math.round(m.pnl).toLocaleString(); stats[3].parentElement.className = pnlClass; }
  if (stats[4]) { stats[4].textContent = m.totalInvest > 0 ? (m.profitRate.toFixed(1) + '%') : '-'; stats[4].parentElement.className = pnlClass; }
  ...
  if (tfoot) tfoot.textContent = Math.round(m.totalInvest).toLocaleString();
  if (tfoot2) tfoot2.textContent = Math.round(m.totalShare).toLocaleString();
}
  ...
  return `
    <div class="fund" style="border-top: 4px solid ${f.color}">
        ...
        <div class="ring-left">
          <div class="param-panel">
            ...
            <div class="param-list">
              <div class="param-row"><span class="param-lbl">基准价</span><input type="number" step="0.0001" id="base-basePrice-${i}" value="${f.basePrice}" class="param-input"></div>
              ...
              <div class="param-row highlight"><span class="param-lbl">初始成本</span><input type="number" step="0.0001" id="base-initCost-${i}" value="${f.initCost || 0}" class="param-input"></div>
        ...
        <div class="ring-center">
          ${(() => {
            ...
            return `
              ...
              <div class="ring-foot">剩余 ${Math.max(0, f.target - m.totalInvest).toLocaleString()}</div>
            `;
          })()}
        </div>
      </div>
      <div class="hold-panel">
        <div class="hold-grid">
          <div class="hold-cell"><div class="hold-lbl">持有金额</div><div class="hold-val">${m.marketValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div></div>
          <div class="hold-cell"><div class="hold-lbl">持有份额</div><div class="hold-val">${m.totalShare.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div></div>
          <div class="hold-cell"><div class="hold-lbl">持仓成本</div><div class="hold-val">${m.avgCost > 0 ? m.avgCost.toFixed(4) : '-'}</div></div>
          <div class="hold-cell ${pnlClass}"><div class="hold-lbl">持有收益</div><div class="hold-val">${(m.pnl >= 0 ? '+' : '') + m.pnl.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div></div>
          <div class="hold-cell ${pnlClass}"><div class="hold-lbl">持有收益率</div><div class="hold-val">${m.totalInvest > 0 ? (m.profitRate.toFixed(2) + '%') : '-'}</div></div>
        </div>
      </div>
          ...
          <table class="buy-table">
            ...
            <tfoot>
              <tr>
                ...
                <td><b>${Math.round(m.totalInvest).toLocaleString()}</b></td>
                <td></td>
              </tr>
            </tfoot>
          </table>
...
function showExportModal() {
  ...
  const stats = state.map(f => ({ f, ...calcFundMetrics(f) }));
  const totalInvested = stats.reduce((s, x) => s + x.totalInvest, 0);
  const totalShares = stats.reduce((s, x) => s + x.totalShare, 0);
  ...
  const summaryRows = stats.map(s => {
    const ratio = totalInvested > 0 ? (s.totalInvest / totalInvested * 100) : 0;
    return `<tr>
      ...
      <td>${Math.round(s.totalShare).toLocaleString()}</td>
      <td>${s.avgCost > 0 ? s.avgCost.toFixed(4) : '-'}</td>
      <td style="color:${pnlColor(s.pnl)}">${pnlSign(s.pnl)}${Math.round(s.pnl).toLocaleString()}</td>
      <td style="color:${pnlColor(s.profitRate)}">${s.profitRate.toFixed(2)}%</td>
      <td>${Math.round(s.totalInvest).toLocaleString()}</td>
  ...
  state.forEach(f => {
    f.buys.forEach(b => {
      const sh = Number(b.price || 0) > 0 ? b.amount / b.price : 0;
      buyRows.push(`<tr>
        <td>${f.name}</td>
        <td>${b.date}</td>
...
function exportExcelToFile() {
  ...
  const totalShares = state.reduce((s, f) => s + calcFundMetrics(f).totalShare, 0);
  const rows = [
    ['基金加仓总览', '', '', '', '', '', '', '', '', '', '', '', ''],
  ...
  state.forEach(f => {
    f.buys.forEach(b => {
      const sh = Number(b.price || 0) > 0 ? (b.amount / b.price) : 0;
      rows.push([f.name, b.date, b.tier || 0, b.price, b.amount ? Math.round(b.amount) : '', sh ? sh.toFixed(2) : '', '', '', '', '', '', '']);
    });
  });
...
function saveAsExcel() {
  ...
  const totalPnl = totalVal - totalInv;
  const totalRate = totalInv > 0 ? (totalPnl / totalInv * 100) : 0;
  const totalShares = state.reduce((s, f) => calcFundMetrics(f).totalShare, 0);
  const rows = [
    ['基金加仓总览', '', '', '', '', '', '', '', '', '', '', '', ''],
