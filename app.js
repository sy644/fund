...
function bindFundEvents(f, i) {
  ...
  // 长按左滑删除手势 - 整段委托到 body 上(只注册一次)
  (function setupSwipeToDelete() {
    if (document.body.dataset.swipeBound === '1') return;
    document.body.dataset.swipeBound = '1';
    let activeRow = null;
    const MAX = 80;        // 滑动最大距离
    const THRESHOLD = 40;  // 露出阈值
    const LONG_PRESS_MS = 500; // 长按触发时间
    const setX = (row, x) => {
      const inner = row.querySelector('.buy-row-inner');
      if (!inner) return;
    ...
    const close = (row) => {
      ...
      if (row._longPressTimer) {
        clearTimeout(row._longPressTimer);
        row._longPressTimer = null;
      }
      if (activeRow === row) activeRow = null;
    };
    document.body.addEventListener('touchstart', e => {
      const row = e.target.closest('.buy-row');
      if (activeRow && row !== activeRow) close(activeRow);
      if (row) {
        const t = e.touches[0];
        row._swipeStartX = t.clientX;
        row._swipeStartY = t.clientY;
        row._swipeAxis = null;
        row._swiping = true;
        // 长按定时器: 500ms 后该行进入"可删除"模式
        ...
        // 长按进度环: 0 -> 1
        row._lpStart = Date.now();
        row.classList.add('pressing');
        requestAnimationFrame(function lpProgress(ts) {
          if (!row._swiping) return;
          if (row.classList.contains('ready-delete')) {
            row.classList.remove('pressing');
            row.style.setProperty('--lp-progress', '1');
            return;
          }
          const elapsed = Date.now() - (row._lpStart || Date.now());
          const p = Math.min(1, elapsed / LONG_PRESS_MS);
          row.style.setProperty('--lp-progress', p.toFixed(3));
          if (p < 1) {
            requestAnimationFrame(lpProgress);
          }
        });
      }
    }, {passive: true});
    document.body.addEventListener('touchmove', e => {
      const row = e.target.closest('.buy-row');
      if (!row || !row._swiping) return;
      // 一旦移动就取消长按计时
      const t = e.touches[0];
      const dx = t.clientX - row._swipeStartX;
      const dy = t.clientY - row._swipeStartY;
      ...
      // 只有当行处于 ready-delete 状态, 左滑才生效
      if (!row.classList.contains('ready-delete')) return;
      if (!row._swipeAxis) {
        if (Math.abs(dx) > 6 || Math.abs(dy) > 6) {
          row._swipeAxis = Math.abs(dx) > Math.abs(dy) ? 'x' : 'y';
        }
      }
      if (row._swipeAxis === 'x') {
        const cur = row._curX || 0;
        setX(row, cur + dx);
      }
    }, {passive: true});
    document.body.addEventListener('touchend', e => {
      ...
      const axis = row._swipeAxis;
      row._swipeAxis = null;
      // 如果没进入 ready-delete 模式, 复位并退出
      if (!row.classList.contains('ready-delete')) {
        row.style.setProperty('--lp-progress', '0');
        close(row);
        return;
      ...
      const cur = row._curX || 0;
      if (axis === 'x' && cur < -THRESHOLD) {
        const inner = row.querySelector('.buy-row-inner');
        if (inner) inner.style.transform = `translateX(-${MAX}px)`;
        row._curX = -MAX;
    ...
    // 点击/触摸页面其它区域时, 关闭所有"ready-delete / swiped"状态
    document.body.addEventListener('touchstart', e => {
      if (!e.target.closest('.buy-row')) {
        document.querySelectorAll('.buy-row.ready-delete, .buy-row.swiped').forEach(r => close(r));
      }
    }, {passive: true});
  })();
  ...
  return `
    <div class="fund" style="border-top: 4px solid ${f.color}">
      ...
      <div class="ring-section">
        <div class="ring-left" style="min-width:0;max-width:100%;overflow:hidden;position:relative;z-index:2">
          <div class="param-panel" style="max-width:100%;min-width:0;overflow:hidden;position:relative;z-index:2">
            <div class="param-panel-head">参数设置 ›</div>
            <div class="param-list">
              <div class="param-row"><span class="param-lbl">基准</span><input type="number" step="0.0001" id="base-basePrice-${i}" value="${f.basePrice}" class="param-input" style="width:100%;min-width:0;max-width:100%;font-size:13px;box-sizing:border-box;overflow:hidden"></div>
              <div class="param-row highlight"><span class="param-lbl">初始份额</span><input type="number" step="1" id="base-initShares-${i}" value="${f.initShares}" class="param-input" style="width:100%;min-width:0;max-width:100%;font-size:13px;box-sizing:border-box;overflow:hidden"></div>
              <div class="param-row"><span class="param-lbl">目标</span><input type="number" step="100" id="base-target-${i}" value="${f.target}" class="param-input" style="width:100%;min-width:0;max-width:100%;font-size:13px;box-sizing:border-box;overflow:hidden"></div>
              <div class="param-row"><span class="param-lbl">中</span><input type="number" step="0.0001" id="price-priceMid-${i}" value="${f.priceMid||0}" class="param-input" title="中点" style="width:100%;min-width:0;max-width:100%;font-size:13px;box-sizing:border-box;overflow:hidden"></div>
              <div class="param-row"><span class="param-lbl">低</span><input type="number" step="0.0001" id="price-priceLow-${i}" value="${f.priceLow||0}" class="param-input" title="低点" style="width:100%;min-width:0;max-width:100%;font-size:13px;box-sizing:border-box;overflow:hidden"></div>
              <div class="param-row"><span class="param-lbl">高</span><input type="number" step="0.0001" id="price-priceHigh-${i}" value="${f.priceHigh||0}" class="param-input" title="高点" style="width:100%;min-width:0;max-width:100%;font-size:13px;box-sizing:border-box;overflow:hidden"></div>
            </div>
          </div>
        </div>
      ...
      <div class="hold-panel">
        <div class="hold-grid">
          ...
          <div class="hold-cell pnl-flash" style="color:${pnl>=0?'#dc2626':(pnl<0?'#16a34a':'#93A3BD')};background:${pnl>=0?'rgba(220,38,38,0.12)':(pnl<0?'rgba(22,163,74,0.12)':'transparent')};border-radius:10px;padding:6px 4px;animation:${Math.abs(pnl)>0?'pnlPulse 1.6s ease-in-out infinite':'none'};--pnl-color:${pnl>=0?'#dc2626':(pnl<0?'#16a34a':'#93A3BD')}"><div class="hold-lbl" style="color:#93A3BD">持有收益</div><div class="hold-val" style="text-shadow:0 0 10px var(--pnl-color)">${(pnl>=0?'+':'')+pnl.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</div></div>
          <div class="hold-cell pnl-flash" style="color:${pnl>=0?'#dc2626':(pnl<0?'#16a34a':'#93A3BD')};background:${pnl>=0?'rgba(220,38,38,0.12)':(pnl<0?'rgba(22,163,74,0.12)':'transparent')};border-radius:10px;padding:6px 4px;animation:${Math.abs(pnl)>0?'pnlPulse 1.6s ease-in-out infinite':'none'};--pnl-color:${pnl>=0?'#dc2626':(pnl<0?'#16a34a':'#93A3BD')}"><div class="hold-lbl" style="color:#93A3BD">持有收益率</div><div class="hold-val" style="text-shadow:0 0 10px var(--pnl-color)">${invested > 0 ? ((pnl/invested*100).toFixed(2) + '%') : '-'}</div></div>
        </div>
      </div>
      <div class="buy-section">
        <div class="section-title">
          📋 交易记录
          <div class="buy-btns">
            <button class="buy-toggle-btn" id="delToggle-${i}" title="删除模式">−</button>
        ...
        <div class="buy-table-wrap">
          ...
          <div class="buy-grid-body">
            ...
            <div class="buy-row" data-bi="${bi}" data-fund-i="${i}">
              ...
              <div class="buy-row-action" data-buy-del="${i}" data-idx="${bi}">
                <button class="del-btn" data-buy-del="${i}" data-idx="${bi}">×</button>
                <span class="del-hint">左滑删除</span>
              </div>
            </div>
              `;}).join('')}
          </div>
