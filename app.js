...
function buildTierTable(f) {
  ...
  for (let t = 10; t >= -10; t--) {
    ...
    if (t === 0) {
      // 基准行: 加仓金额 = 等比数列的 buyEnd 档金额
      // 总档数 tiers(包含基准), buyStart 档是初期(r=1), buyEnd 档是 r=tiers-1
      // 所以基准行 = m1 * multi^(tiers - 2)
      amt = m1 * Math.pow(multi, tiers - 2);
      label = '基准';
      trigger = basePrice;
    } else {
        ...
        <div class="ring-left" style="min-width:0;max-width:100%;overflow:hidden;position:relative;z-index:2">
          <div class="param-panel" style="max-width:100%;min-width:0;overflow:hidden;position:relative;z-index:2">
            ...
            <div class="param-list">
              <div class="param-row" style="grid-area: 1 / 1"><input type="number" step="0.0001" id="base-basePrice-${i}" value="${f.basePrice}" class="param-input" style="width:100%;min-width:0;max-width:100%;font-size:13px;box-sizing:border-box;overflow:hidden"></div>
              <div class="param-row highlight" style="grid-area: 2 / 1"><input type="number" step="1" id="base-initShares-${i}" value="${f.initShares}" class="param-input" style="width:100%;min-width:0;max-width:100%;font-size:13px;box-sizing:border-box;overflow:hidden"></div>
              <div class="param-row" style="grid-area: 3 / 1"><input type="number" step="100" id="base-target-${i}" value="${f.target}" class="param-input" style="width:100%;min-width:0;max-width:100%;font-size:13px;box-sizing:border-box;overflow:hidden"></div>
              <div class="param-row" style="grid-area: 1 / 2"><input type="number" step="0.0001" id="price-priceHigh-${i}" value="${f.priceHigh||0}" class="param-input" title="高点" style="width:100%;min-width:0;max-width:100%;font-size:13px;box-sizing:border-box;overflow:hidden"></div>
              <div class="param-row" style="grid-area: 2 / 2"><input type="number" step="0.0001" id="price-priceMid-${i}" value="${f.priceMid||0}" class="param-input" title="中点" style="width:100%;min-width:0;max-width:100%;font-size:13px;box-sizing:border-box;overflow:hidden"></div>
              <div class="param-row" style="grid-area: 3 / 2"><input type="number" step="0.0001" id="price-priceLow-${i}" value="${f.priceLow||0}" class="param-input" title="低点" style="width:100%;min-width:0;max-width:100%;font-size:13px;box-sizing:border-box;overflow:hidden"></div>
            </div>
          </div>
        </div>
