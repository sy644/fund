  ...
  f.buys.forEach((b, bi) => {
    ...
    if (sdayInp) {
      ...
      sdayInp.addEventListener('change', e => {
        ...
        // Sday 选了但匹配不到 → 弹窗提示补录
        if (sday && sdayNav == null && oldSday !== sday) {
          showModal({
            title: '净值缺失',
            message: '该日期 [' + sday + '] 没有 [ ' + f.name + ' ] 的净值记录。\n是否现在添加?',
            okText: '添加净值',
            cancelText: '取消',
          }).then(ok => {
            if (ok) {
              showAddNavDialog(f.code, f.name, sday);
            }
          });
        }
      });
          ...
          <div class="buy-grid-body">
            ...
            <div class="buy-row" data-bi="${bi}" data-fund-i="${i}">
              <div class="buy-row-inner">
                ...
                <div class="bc bc-pill bc-sday"><input type="date" id="bsday-${i}-${bi}" value="${sday}" max="${new Date().toISOString().split('T')[0]}" data-short="${sdayShort}" class="bcell bdate-slider" data-bi="${bi}" data-fund-i="${i}"></div>
              </div>
            </div>
              `;}).join('')}
          </div>
