...
function render() {
  ...
  html += '<button class="dock-btn tab-summary ' + (activeTab===state.length?'active':'') + '" data-tab="' + state.length + '" title="汇总"><span class="dock-ico">📊</span><span class="dock-lbl">汇总</span></button>';
  state.forEach((f, i) => {
    html += '<button class="dock-btn ' + (i===activeTab?'active':'') + '" data-tab="' + i + '" title="' + f.name + '"><span class="dock-ico">📈</span><span class="dock-lbl">' + f.name + '</button>';
  });
  html += '<button class="dock-btn dock-add" data-add="1" title="新增基金"><span class="dock-ico">+</span></button>';
  html += '</div>';
  ...
  return `
    <div class="fund" style="border-top: 4px solid ${f.color}">
      ...
      <div class="param-section">
        <div class="param-strip" style="justify-content:center;gap:14px;padding:8px 12px">
