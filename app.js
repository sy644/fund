...
(function injectAnimStyles() {
  ...
  s.textContent = `
    ...
    .buy-row.hl-yellow { background:rgba(251,191,36,0.18); border-left:3px solid #fbbf24; box-shadow:inset 0 0 12px rgba(251,191,36,0.15); }
    .buy-row.hl-green { background:rgba(34,197,94,0.18); border-left:3px solid #22c55e; box-shadow:inset 0 0 12px rgba(34,197,94,0.18); }
    .buy-row .hl-close { position:absolute; right:6px; top:50%; transform:translateY(-50%); width:18px; height:18px; border-radius:50%; display:flex; align-items:center; justify-content:center; background:rgba(0,0,0,0.55); color:#fff; font-size:12px; font-weight:900; line-height:1; cursor:pointer; z-index:5; box-shadow:0 0 6px rgba(0,0,0,0.4); user-select:none; }
    .buy-row .hl-close:hover { background:rgba(0,0,0,0.85); }
...
function updateRangeTrack(track) {
  ...
  if (f) {
    ...
    // 收益 = (Sday 净值 - Bday 净值) × Bday 份额, 只算 Sday 非空的
  ...
  f.buys.forEach((b, bi) => {
    ...
    if (amtInp) {
      amtInp.addEventListener('input', e => {
        ...
        if (b._highlight) {
          delete b._highlight;
          var row = document.querySelector(`.buy-row[data-bi="${bi}"][data-fund-i="${i}"]`);
          if (row) {
            row.classList.remove('hl-yellow', 'hl-green');
            var hc = row.querySelector('.hl-close');
            if (hc) hc.remove();
          }
        }
        refreshAmtColor();
    ...
    var hlClose = document.querySelector(`.buy-row[data-bi="${bi}"][data-fund-i="${i}"] .hl-close`);
    if (hlClose) hlClose.addEventListener('click', (e) => {
      e.stopPropagation();
      e.preventDefault();
        ...
        <div class="buy-table-wrap">
          ...
          <div class="buy-grid-body">
              ${f.buys.map((b, bi) => {
                ...
                return `
            <div class="buy-row ${b._highlight === 'yellow' ? 'hl-yellow' : ''} ${b._highlight === 'green' ? 'hl-green' : ''}" data-bi="${bi}" data-fund-i="${i}">
              <div class="buy-row-inner">
                ${b._highlight ? `<div class="hl-close" data-bi="${bi}" data-fund-i="${i}" title="清除高亮">×</div>` : ''}
          ...
          <!-- 修改：合计金额和份额仅统计交易记录 (inv_buys, sh_buys) -->
...
function addBuyByAmountDialog(i) {
  ...
  showModal({
    ...
    if (!amt || amt <= 0) { alert('金额要大于 0'); return; }
    var prev = JSON.stringify(state);
    f.buys.push({
      date: bday,
      type: 'buy',
      ...
      amount: amt,
      tier: 0,
      _highlight: 'yellow', // 标记: 黄色
    });
    save(prev);
    render();
    flashHint('🟡 已录入金额 ' + Math.round(amt).toLocaleString() + ' / 份额 ' + (nav > 0 ? (amt / nav).toFixed(2) : '-'));
  });
}

// 点 − : 录入份额 → 用 bday 匹配到的净值算金额, 整行标记绿, amount 留负号
function subBuyBySharesDialog(i) {
  ...
  showModal({
    ...
    if (!sh || sh <= 0) { alert('份额要大于 0'); return; }
    if (nav <= 0) { alert('当前无净值, 无法计算金额'); return; }
    var amt = sh * nav; // 正数, 存的时候取负
    var negAmt = -Math.abs(amt);
    var prev = JSON.stringify(state);
    f.buys.push({
      date: bday,
      type: 'sell',
      ...
      amount: negAmt, // 保留负号
      tier: 0,
      _highlight: 'green', // 标记: 绿色
    });
    save(prev);
    render();
    flashHint('🟢 已录入份额 ' + sh.toFixed(2) + ' / 金额 ' + Math.round(negAmt).toLocaleString());
  });
}
