...
function bindFundEvents(f, i) {
  ...
  f.buys.forEach((b, bi) => {
    ...
    var refreshAmtColor = () => {
      if (!amtInp) return;
      // 有 _mark 时让 CSS 接管, 不要写内联 color
      if (b._mark) {
        amtInp.style.color = 'inherit';
        return;
      }
      var v = b.amount || 0;
        ...
        <div class="buy-table-wrap">
          ...
          <div class="buy-grid-body">
              ${f.buys.map((b, bi) => {
                ...
                var shares = (b._shares != null) ? Math.abs(b._shares) : ((realAmt && bNavMatch) ? (Math.abs(realAmt) / bNavMatch) : 0);
                var priceNow = f.price || 0;
                var priceBuy = bNavMatch != null ? bNavMatch : 0;
            ...
            <div class="buy-row" data-bi="${bi}" data-fund-i="${i}">
              <div class="buy-row-inner">
                <div class="bc bc-pill bc-date ${b.date && !navHistory.find(r => r.code === f.code && r.date === b.date) ? 'sday-miss' : ''}" data-mark="${b._mark || ''}"><input type="date" id="bdate-${i}-${bi}" value="${b.date||''}" data-short="${dateShort}" class="bcell bdate-slider"></div>
                <div class="bc bc-pill bc-nav ${!bNavMatch ? 'sday-miss' : ''}" id="bnavwrap-${i}-${bi}" data-mark="${b._mark || ''}">${bNavMatch != null ? '<span class="bnav-readonly" data-bi="'+bi+'" style="color:inherit;font-size:12px;font-weight:700;font-family:monospace">'+bNavMatch.toFixed(4)+'</span>' : '<span class="bnav-readonly" data-bi="'+bi+'" style="color:inherit;font-size:11px;font-weight:600">无匹配</span>'}</div>
                ...
                <div class="bc bc-pill" data-mark="${b._mark || ''}"><span class="bshares" data-bi="${bi}" style="color:inherit;font-size:13px;font-weight:700">${shares ? shares.toFixed(2) : '-'}</span></div>
...
function parseBuyRecords(text) {
  ...
  // 卖出/赎回关键词 (含 OCR 常见错字: 售出, 赎买, 卖入, 卖购)
  var reSell = /(卖出|赎回|售出|卖入|卖购|redeem|redemption|sell)/i;
  // 买入关键词 (含 OCR 常见错字)
  var reBuy = /(买入|申购|认购|购入|买购|买入金额|buy|purchase)/i;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    var type = reSell.test(ctx) ? 'sell' : (reBuy.test(ctx) ? 'buy' : 'buy');
...
function showOCRConfirmDialog(f, i, records) {
  ...
  box.querySelector('#ocrOk').onclick = () => {
    ...
    records.forEach(r => {
      ...
      // 与 + / − 按钮逻辑一致: 有净值就算, 无净值留空
      var newBuy = {
        date: r.bday,
        type: isSell ? 'sell' : 'buy',
        price: nav,
        amount: 0, // 卖出无匹配时金额为 0, 渲染显示 -
        tier: 0,
        _mark: isSell ? 'green' : 'red', // 卖出绿, 买入红
      };
      if (isSell) {
        // 卖出: 截图给的是金额, 算份额(用负数存, 持仓汇总直接累加)
        if (nav > 0) {
          newBuy._shares = -(r.amount / nav);
          newBuy.amount = -r.amount; // 金额也存负(与 − 按钮一致)
          hitCount++;
        } else {
          missCount++;
        }
...
function addBuyByAmountDialog(i) {
  ...
  showModal({
    ...
    var newBuy = {
      ...
      amount: amt,
      tier: 0,
      _mark: 'red', // 🔴 红色: Bday/净值/金额/份额
    };
    if (nav > 0) {
      // 有净值, 自动算份额
      newBuy._shares = amt / nav;
    }
    // 无净值: 不存 _shares, 渲染时显示 -
    f.buys.push(newBuy);
    save(prev);
    render();
    if (nav > 0) {
...
function subBuyBySharesDialog(i) {
  ...
  showModal({
    ...
    var newBuy = {
      ...
      type: 'sell',
      price: nav, // 0 表示无匹配
      amount: 0,  // 无匹配时金额留 0, 渲染显示 -
      tier: 0,
      _shares: -Math.abs(sh), // 卖出份额为负, 持仓汇总时直接累加
      _mark: 'green', // 🟢 绿色: Bday/净值/金额/份额
    };
    if (nav > 0) {
      // 有净值: 算金额 (存负)
